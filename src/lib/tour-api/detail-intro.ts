...
const SUPPORTED_CONTENT_TYPE_IDS = [
  "12",
  "14",
  "39",
] as const;
...
export const getTourDetailIntro = async ({
  contentId,
  contentTypeId,
}: GetTourDetailIntroParams): Promise<
  InformationItem[]
> => {
  const normalizedContentId =
    contentId.trim();

  const normalizedContentTypeId =
    contentTypeId.trim();
  ...
  if (
    ...
    throw new Error(
      `Unsupported contentTypeId: ${normalizedContentTypeId}`,
    );
  }
  ...
  const parsed =
    tourDetailIntroResponseSchema.parse(
      rawData,
    );
  ...
  return normalizeTourDetailIntroItem(
    item,
  );
};